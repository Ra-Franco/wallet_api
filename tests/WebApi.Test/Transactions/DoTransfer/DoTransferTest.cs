...
namespace WebApi.Test.Transactions.DoTransfer
{
    public class DoTransferTest : WalletCustomClassFixture
    {
        private readonly string ROUTE = "transaction/transfer";
        ...
        public async Task Success()
        {
            ...
            request.ReceiverCpf = _cpfReceiver;
        ...
        public async Task Error_Amount_Invalid(string culture)
        {
            ...
            request.TransactionPassword = _transactionalPassword;
            request.ReceiverCpf = _cpfReceiver;
            request.Amount = decimal.MaxValue;
        ...
        [Theory]
        [ClassData(typeof(CultureInlineDataTest))]
        public async Task Error_Cpf_Not_Found(string culture)
        {
            var token = JwtTokenGeneratorBuilder.Build().Generate(_userIdentifier);
            var request = RequestTransferBuilder.Build();

            request.TransactionPassword = _transactionalPassword;
            request.ReceiverCpf = _faker.Person.Cpf();
            ...
            var expectedMessage = ResourceMessageException.ResourceManager.GetString("RECEIVER_WALLET_NOT_FOUND", new CultureInfo(culture));
